  ...
  /* Functions below here need to be implemented */
	...
	*/
	...
	//TODO
		...
		/*Task 4*/
		while(i<n-1){
			poss(i)(i+1)(op(w(i))(w(i+1)))=true
			poss(i)(i+1)((op(w(i))(w(i+1))+1)%3)=false
			poss(i)(i+1)((op(w(i))(w(i+1))+2)%3)=false
			i += 1
		}
		while(k<n){
			i=0
		...
		var ways = Array.ofDim[Int](MAXWORD, MAXWORD, 3)
		while(i<n-1){
			ways(i)(i+1)(op(w(i))(w(i+1)))=1
			ways(i)(i+1)((op(w(i))(w(i+1))+1)%3)=0
			ways(i)(i+1)((op(w(i))(w(i+1))+2)%3)=0
			i += 1
		}
		k=2;
		while(k<n){
